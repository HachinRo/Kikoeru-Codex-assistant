...
| ASMR commands | `~/.hermes/asmr/bin` | `asmr-view`, `asmr-library`, `asmr-subs`, `asmr-neo`, `neokikoeru-serve`. |
| ASMR library code | `~/.hermes/asmr/lib` | Includes `asmr_view_kikoeru.py`, the main viewer backend. |
| Kikoeru SPA | `~/.hermes/asmr/kikoeru-spa` | Compiled frontend bundle; patches are brittle. |
| Media root | `/Volumes/TOSHIBA/AMSR` | Do not bulk-mutate media casually. |
| NeoKikoeru DB | `~/Library/Application Support/neokikoeru/neokikoeru.db` | Treat as live state. Back up before direct DB edits. |

## Important files

- `~/.hermes/asmr/bin/asmr-view`: process manager / entrypoint for port `8890`.
- `~/.hermes/asmr/bin/asmr-dashboard`: integrated/standalone dashboard server logic.
- `~/.hermes/asmr/bin/asmr-library`: umbrella CLI for build, dashboard, works, and subtitles.
- `~/.hermes/asmr/bin/asmr-subs`: subtitle download/refetch/encoding CLI.
- `~/.hermes/asmr/bin/asmr-neo`: work/file lookup CLI now expected to use viewer API/stream routes on `8890`.
- `~/.hermes/asmr/bin/neokikoeru-serve`: NeoKikoeru adapter; previously had a float progress formatting crash in tasks output.
- `~/.hermes/asmr/lib/asmr_view_kikoeru.py`: Kikoeru-compatible routes, media streaming, search, grouping.
- `~/.hermes/asmr/kikoeru-spa/js/app.051b603f.fix2.js`: patched SPA bundle.
- `~/.hermes/asmr/kikoeru-spa/index.html`: points the frontend to the patched bundle.
