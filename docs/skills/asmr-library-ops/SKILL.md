...
Work on the local ASMR stack with the assumption that it is a live media service. Preserve media, databases, and user edits. Prefer read-only checks before repairs, then verify through the local HTTP routes the user actually sees.

Always prefix shell commands with `rtk` per the workspace instructions, except when debugging the wrapper itself or when a script must be run through an interpreter directly.
...
- Do not start a standalone dashboard on `8891`; `asmr-library dashboard` should point to the integrated dashboard.
...
- Local reference repos: `refs/kikoeru-express`, `refs/asmr-downloader`, `refs/neokikoeru`
...
Use `asmr-view` as the owning process for port `8890`. This local build uses `stop` + `start` rather than a `restart` subcommand. Check status first, then restart cleanly and verify `/`, `/asmr-library`, and one API route.
...
Verify these route families after changes:
...
- Patch source-like Python/CLI files under `~/.hermes/asmr/bin` and `~/.hermes/asmr/lib` carefully; these are the live service files.
...
- Never modify media files or the NeoKikoeru database unless the task explicitly requires it and the operation is understood.
- Keep cloned upstream repos as references unless the user asks to port source-level changes.
- After every repair, verify with local HTTP endpoints and one realistic work/RJ example.
...
- Tag example: `/api/tags/500/works` returns tag `舔耳` with many works.
