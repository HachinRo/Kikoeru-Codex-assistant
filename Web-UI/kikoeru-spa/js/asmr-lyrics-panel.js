(function () {
  ...
  function getStore () {
    ...
    if (store) patchStoreCommit(store)
    return store
  }

  function patchStoreCommit (store) {
    if (!store || store.__asmrLyricsCommitPatched || typeof store.commit !== 'function') return
    var originalCommit = store.commit.bind(store)
    store.__asmrLyricsCommitPatched = true
    store.__asmrLyricsOriginalCommit = originalCommit
    store.commit = function (type, payload, options) {
      var mutation = typeof type === 'object' && type ? type.type : type
      if (mutation === 'AudioPlayer/SET_CURRENT_LYRIC' && !state.allowLyricCommit) return
      return originalCommit(type, payload, options)
    }
  }
  ...
  function commitCurrentLyric () {
    ...
    state.allowLyricCommit = true
    try {
      store.commit('AudioPlayer/SET_CURRENT_LYRIC', text)
    } finally {
      state.allowLyricCommit = false
    }
  }
  ...
  function loadSubtitleTitle (hash, token) {
    ...
    return freshFetch('/api/track/' + encodeURIComponent(clean) + token)
  ...
  function loadLyricsForCurrentTrack (force) {
    var hash = currentHash()
    var track = getCurrentTrack()
    if (!hash || (!force && hash === state.hash)) return
    ...
    loadTrackMeta(hash, track, token)
      .then(function (meta) {
        ...
        state.offset = loadOffset()
        updateOffsetInput()
        return freshFetch('/api/media/check-lrc/' + hash + token)
      })
      .then(function (res) {
        if (!res) return null
        if (!res.ok) throw new Error(res.status + ' ' + res.statusText)
